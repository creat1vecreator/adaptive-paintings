...
function Pagination({
  ...
  const certainPageHandler = (evt) => {
    setPages({ ...pages, currentPage: +evt.target.value });
  };
    ...
    <div className={styles.pagination__wrapper} data-theme={theme}>
      <ul className="pagination">
        <button
          ...
          disabled={pages.currentPage === 1 || !totalButtons.length}
        ...
        >
          ...
          >
            <path
              ...
              fillOpacity={pages.currentPage === 1 || !totalButtons.length ? 0.3 : 1}
            />
          </svg>
...

        {
            totalButtons.map((num) => ((
              <button
                className={`page-item ${pages.currentPage === num ? styles.current : ''}`}
                key={`key-${num}`}
                value={num}
        ...
        <button
          className="ahead"
          disabled={pages.currentPage === totalButtons[totalButtons.length - 1]}
          onClick={aheadPage}
        >
          <svg
            className="ahead"
          ...
          >
            <path
              ...
              fill={theme === 'light' ? 'black' : 'white'}
              fillOpacity={pages.currentPage === totalButtons[totalButtons.length - 1] || !totalButtons.length ? '0.3' : '1'}
            />
          </svg>
        </button>
        ...
        <button
          ...
          disabled={pages.currentPage === totalButtons[totalButtons.length - 1] || !totalButtons.length}
